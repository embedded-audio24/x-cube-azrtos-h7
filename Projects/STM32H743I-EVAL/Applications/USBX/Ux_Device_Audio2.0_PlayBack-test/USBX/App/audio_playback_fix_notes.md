...
2. **Draining playback gracefully.** When the host closes the stream we wait briefly for the DMA engine to finish the queued samples before stopping the codec and we reset the buffer state. We now poll the buffered byte count directly so the drain completes even if the playback flag drops while the DMA is still flushing. This keeps the media player from hanging on completion while ensuring the next playback starts from silence.
