...
2. **Draining playback gracefully.** When the host closes the stream we mute the codec and hand the drain work to the playback thread so the USB control request returns immediately. The thread clears any pending samples to silence, waits briefly for the DMA engine to finish the zeroed audio, lets the mute settle for a couple of milliseconds, and finally stops the SAI (only if it is still running) before resetting the buffer state while signalling completion back to the control side. The control handler now waits for that completion both when closing and when immediately reopening the stream so the next track does not compete with an in-progress drain. Silencing the tail removes the post-playback click while the asynchronous drain keeps the media player responsive and still guarantees the next playback starts from silence.
