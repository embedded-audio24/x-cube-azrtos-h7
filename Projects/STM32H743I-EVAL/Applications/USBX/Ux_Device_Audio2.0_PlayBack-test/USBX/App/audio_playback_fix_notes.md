...
2. **Draining playback gracefully.** When the host closes the stream we mute the codec, wait briefly for the DMA engine to finish the queued samples before stopping it, and reset the buffer state. We now poll the buffered byte count directly so the drain completes even if the playback flag drops while the DMA is still flushing. Muting first removes the post-playback click while still keeping the media player from hanging on completion and ensuring the next playback starts from silence.
