...
static const uint32_t audio_buffer_half_size = AUDIO_TOTAL_BUF_SIZE / 2U;

void BSP_AUDIO_OUT_HalfTransfer_CallBack(uint32_t Instance)
{
  /* Silence the first half of the playback buffer once it has been played. */
  (void)Instance;
  memset(&BufferCtl.buff[0], 0, audio_buffer_half_size);
}

void BSP_AUDIO_OUT_TransferComplete_CallBack(uint32_t Instance)
{
  /* Silence the second half of the playback buffer once it has been played. */
  (void)Instance;
  memset(&BufferCtl.buff[audio_buffer_half_size], 0, audio_buffer_half_size);
}
                                     ...
                                     ULONG alternate_setting)
{
  ...
  /* Stop playback and reset the local buffer when the host closes the stream. */
  if (alternate_setting == 0U)
  {
    ...
    memset(BufferCtl.buff, 0, AUDIO_TOTAL_BUF_SIZE);

    BSP_AUDIO_OUT_Stop(0);
    tx_queue_flush(&ux_app_MsgQueue);

    return;
  }

  /* Prepare buffer for a fresh playback session. */
  BufferCtl.rd_enable = 0U;
  BufferCtl.rd_ptr = 0U;
  BufferCtl.wr_ptr = 0U;
  BufferCtl.fptr = 0U;
